...
class InventarioController extends Controller
{
    ...
    {
        $inventario = Inventario::select('inventarios.codigoinv', 'inventarios.fecha', 'inventarios.tipo_movimiento')
            ->distinct()
            ->whereNotNull('codigoinv')
            ->get();
        //  dd($inventario);
        return view('inventario.index', compact('inventario'));
    }

    public function viewDetalle($codInv)
    {
        $inventario = Inventario::select('inventarios.*','productos.nombre')
        ->join('productos','productos.id','=','inventarios.id_producto')
        ->where('inventarios.codigoinv', $codInv)
        ->get();
